...
**提示：egg 在开发期默认启动 [development][egg-development] 插件，修改后端代码后，会自动重启 worker 进程。**
...
> egg 内置了 [urllib] 来方便开发者使用 http 请求。
...
egg 提供了一种快速扩展的方法，在 `app/extend` 目录下提供扩展脚本即可，具体参见 [扩展](../basics/extend.md)。
...
- egg 提供了强大的扩展机制，参见 [插件封装](../advanced/plugin.md)。
- 一个大规模的团队需要遵循一定的约束和约定，在 egg 里我们建议封装适合自己团队的上层框架，参见 [框架封装](../advanced/framework.md)。
- 在 egg 里面，写单元测试其实很简单的事，并且强烈建议。具体参见 [单元测试](../core/test.md)。
